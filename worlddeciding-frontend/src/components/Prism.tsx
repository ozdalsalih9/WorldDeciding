...
export default function Prism({
  ...
  useEffect(() => {
    ...
    const shortEdge = Math.min(window.innerWidth || 1920, window.innerHeight || 1080)
    const inferredLow =
      prefersReducedMotion || cores <= 4 || memory <= 4 || shortEdge < 820
    const resolvedQuality =
      quality === 'auto' ? (inferredLow ? 'low' : 'high') : quality
    const steps =
      resolvedQuality === 'high' ? 100 : resolvedQuality === 'medium' ? 72 : 48
    const dprCap =
      resolvedQuality === 'high' ? 2 : resolvedQuality === 'medium' ? 1.5 : 1
